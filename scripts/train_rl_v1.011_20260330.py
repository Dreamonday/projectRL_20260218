...
# -*- coding: utf-8 -*-
"""
RL 交易模型训练 v1.011 — 与 v1.02 相同的 GPU/训练逻辑 + 固定随机种子 42
版本: v1.011
日期: 20260330

v1.011 变更 (基于 v1.02):
  - 逻辑与 v1.02 一致（GPU RolloutBuffer、epoch 进度打印、完整 episode 收集等）
  - 环境初始化与 PPO 使用固定随机种子 42，与 v1.01 一致，便于可复现实验

使用方式:
    python scripts/train_rl_v1.011_20260330.py --config configs/rl_config_v1.011.yaml
"""
...
# v1.011: 与 v1.01 对齐的固定训练种子（环境 + SB3）
TRAIN_SEED = 42
...
class GPUCompleteEpisodeRolloutBuffer(CompleteEpisodeRolloutBuffer):
    """
    v1.011: GPU 优化的 RolloutBuffer（同 v1.02）
    - observations 存入 GPU
...
def main():
    parser = argparse.ArgumentParser(description="RL 交易模型训练 v1.011 (v1.02 逻辑 + 固定种子42)")
    parser.add_argument("--config", type=str,
                        default=str(project_root / "configs" / "rl_config_v1.011.yaml"))
    ...
    print("=" * 70)
    print(f"  RL 交易模型 v1.011 (v1.02 逻辑 + 固定种子 {TRAIN_SEED}) — {run_ts}")
    print("=" * 70)
    print_gpu_info()
    ...
    print(f"  v1.011: GPU 优化 buffer + 训练进度打印 + 固定种子 {TRAIN_SEED}")
    ...
    output_dir = project_root / "experiments" / f"rl_ppo_v1.011_{run_ts}"
    ...
    print(f"  随机种子: {train_seed} (固定，与 v1.01 一致)")
    ...
    def process_backtest(dataset, ppy, prefix, use_random_start=False):
        ...
        plot_backtest(
            ...
            title=f"{prefix.capitalize()} Backtest (v1.011)",
        )
