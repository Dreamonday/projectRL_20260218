...
v0.71 变更 (基于 v0.7):
  - 回测输出增强: 新增 run_detailed_backtest, 生成包含完整交易明细的 Excel
    (总资产/已实现盈亏/浮动盈亏/前30大持仓·权重·买卖金额/清仓记录)
  - 数据加载 (npz)、环境、网络架构、训练流程与 v0.7 完全相同

使用方式:
    python scripts/train_rl_v0.71_20260310.py --config configs/rl_config_v0.7.yaml
"""
...
# ═══════════════════════════════════════════════════════════════════════════
# 第三部分: Temporal Attention 策略网络 (与 v0.7 相同)
# ═══════════════════════════════════════════════════════════════════════════
...
class TemporalScoringPolicy(ActorCriticPolicy):
    """v0.71 自定义策略: Temporal Attention + Identity action_net"""
...
def main():
    parser = argparse.ArgumentParser(description="RL 交易模型训练 v0.71")
    parser.add_argument("--config", type=str,
                        default=str(project_root / "configs" / "rl_config_v0.71.yaml"))
    ...
    print("=" * 70)
    print(f"  RL 交易模型 v0.71 (Temporal Attention + Fast Load + 详细回测) — {run_ts}")
    print("=" * 70)
    print_gpu_info()
    ...
    # ─── 2. 输出目录 ───
    output_dir = project_root / "experiments" / f"rl_ppo_v0.71_{run_ts}"
    ...
    def process_backtest(dataset, ppy, prefix):
        ...
        plot_backtest(
            ...
            title=f"{prefix.capitalize()} Backtest (v0.71 Temporal Attention)",
        )
        return metrics
