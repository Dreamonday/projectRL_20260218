...
v0.6 变更 (基于 v0.5):
  - 观测扩展为时间窗口 W=10: 模型看到每只股票最近 W 个时间步的特征序列
  - 打分网络升级为 Temporal Attention: 对时间序列做自注意力后再打分
  - 参数量仍然很小 (~16K)

使用方式:
    python scripts/train_rl_v0.6_20260310.py --config configs/rl_config_v0.6.yaml
"""
...
class MultiStockTradingEnvV3(gym.Env):
    ...
    def _get_obs(self):
        ...
        # 时间窗口: [t-W+1, ..., t]
        start = max(0, t - W + 1)
        window = self.market_features[start:t + 1]  # (actual_len, n_stocks, 6)
...
class TemporalScoringPolicy(ActorCriticPolicy):
    """v0.6 自定义策略: Temporal Attention + Identity action_net"""
...
def main():
    parser = argparse.ArgumentParser(description="RL 交易模型训练 v0.6")
    ...
    print("=" * 70)
    print(f"  RL 交易模型 v0.6 (Temporal Attention) — {run_ts}")
    print("=" * 70)
    print_gpu_info()
    ...
    # ─── 4. 输出目录 ───
    output_dir = project_root / "experiments" / f"rl_ppo_v0.6_{run_ts}"
    ...
    def process_backtest(dataset, ppy, prefix):
        ...
        plot_backtest(
            ...
            title=f"{prefix.capitalize()} Backtest (v0.6 Temporal Attention)",
        )
        pd.DataFrame({
