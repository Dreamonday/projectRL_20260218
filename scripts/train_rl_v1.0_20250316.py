...
# -*- coding: utf-8 -*-
"""
RL 交易模型训练 v1.0 — 长期价值投资风格
版本: v1.0
日期: 20250316

v1.0 变更:
  - 随机起始点: 每次 episode 从随机时间开始
  - 随机 episode 长度: 至少 3 年，最长到数据末尾
  - 回报计算: 仅用 episode 结束时 (最终市值-初始资金)/初始资金，年化后作为 reward
  - 中间 step reward=0
  - 验证集: 数据末尾往前 5 年
  - 训练集: 全量数据，但随机起点必须 < 验证集起点

使用方式:
    python scripts/train_rl_v1.0_20250316120000.py --config configs/rl_config_v1.0.yaml
"""
...
def main():
    parser = argparse.ArgumentParser(description="RL 交易模型训练 v1.0 (长期价值投资)")
    parser.add_argument("--config", type=str,
                        default=str(project_root / "configs" / "rl_config_v1.0.yaml"))
    ...
    print("=" * 70)
    print(f"  RL 交易模型 v1.0 (长期价值投资) — {run_ts}")
    print("=" * 70)
    print_gpu_info()
    ...
    output_dir = project_root / "experiments" / f"rl_ppo_v1.0_{run_ts}"
    ...
    def process_backtest(dataset, ppy, prefix, use_random_start=False):
        ...
        plot_backtest(
            ...
            title=f"{prefix.capitalize()} Backtest (v1.0)",
        )
