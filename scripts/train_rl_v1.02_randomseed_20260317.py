...
# -*- coding: utf-8 -*-
"""
RL 交易模型训练 v1.02 — GPU 优化 + 训练进度打印
版本: v1.02
日期: 20260317

v1.02 变更 (基于 v1.01):
  - 训练阶段增加 epoch 级别进度打印，减少长时间无输出感
  - GAE 计算全程在 GPU 上完成，减少 CPU-GPU 传输
  - observations 存入 GPU，训练时无需每 batch 从 CPU 拷贝
  - values、log_probs、advantages、returns 保持在 GPU
  - collect_rollouts 中直接传递 tensor，避免 GPU->CPU->GPU 往返

使用方式:
    python scripts/train_rl_v1.02_20260317.py --config configs/rl_config_v1.02.yaml
"""
...
def main():
    parser = argparse.ArgumentParser(description="RL 交易模型训练 v1.02 (GPU优化+进度打印)")
    parser.add_argument("--config", type=str,
                        default=str(project_root / "configs" / "rl_config_v1.02.yaml"))
    ...
    print("=" * 70)
    print(f"  RL 交易模型 v1.02 (GPU优化+进度打印) — {run_ts}")
    print("=" * 70)
    print_gpu_info()
    ...
    print(f"  v1.02: GPU 优化 buffer + 训练进度打印")
    ...
    output_dir = project_root / "experiments" / f"rl_ppo_v1.02_{run_ts}"
    ...
    def process_backtest(dataset, ppy, prefix, use_random_start=False):
        ...
        plot_backtest(
            ...
            title=f"{prefix.capitalize()} Backtest (v1.02)",
        )
