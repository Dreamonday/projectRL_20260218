...
v0.92 变更 (基于 v0.91):
  - 新增 trade_interval: 支持每月交易一次（方案A）
  - 输入信息不变（仍为前30周数据），仅在交易步执行 action，持有步保持仓位
  - 周频数据下 trade_interval=4 表示每月交易1次，1 表示每周交易1次（与 v0.91 一致）

使用方式:
    python scripts/train_rl_v0.92_20260311.py --config configs/rl_config_v0.92.yaml
...
def main():
    parser = argparse.ArgumentParser(description="RL 交易模型训练 v0.92")
    parser.add_argument("--config", type=str,
                        default=str(project_root / "configs" / "rl_config_v0.92.yaml"))
    ...
    print("=" * 70)
    print(f"  RL 交易模型 v0.92 (每月交易) — {run_ts}")
    print("=" * 70)
    print_gpu_info()
    ...
    output_dir = project_root / "experiments" / f"rl_ppo_v0.92_{run_ts}"
    ...
    def process_backtest(dataset, ppy, prefix):
        ...
        plot_backtest(
            ...
            title=f"{prefix.capitalize()} Backtest (v0.92)",
        )
