...
# -*- coding: utf-8 -*-
"""
RL 交易模型训练 v1.01 — 仅用完整 episode 更新
版本: v1.01
日期: 20250316

v1.01 变更 (基于 v1.0):
  - 仅收集完整 episode 的 transition 用于更新
  - 未结束 episode 的 transition 不加入 buffer
  - 每次加入整个 episode，不中途截断；buffer 容量为 n_steps*2，可容纳完整 episode

使用方式:
    python scripts/train_rl_v1.01_20250316.py --config configs/rl_config_v1.01.yaml
"""
...
def main():
    parser = argparse.ArgumentParser(description="RL 交易模型训练 v1.01 (仅完整episode)")
    parser.add_argument("--config", type=str,
                        default=str(project_root / "configs" / "rl_config_v1.01.yaml"))
    ...
    print("=" * 70)
    print(f"  RL 交易模型 v1.01 (仅完整episode更新) — {run_ts}")
    print("=" * 70)
    print_gpu_info()
    ...
    print(f"  v1.01: 仅收集完整 episode 的 transition 用于更新")
    ...
    output_dir = project_root / "experiments" / f"rl_ppo_v1.01_{run_ts}"
    ...
    train_vec_env = DummyVecEnv([make_env(seed=42)])
    ...
    if best_path.exists():
        print("  恢复早停最佳模型...")
        model = PPOCompleteEpisodes.load(str(model_dir / "ppo_trading_best"), env=train_vec_env)
    ...
    def process_backtest(dataset, ppy, prefix, use_random_start=False):
        ...
        plot_backtest(
            ...
            title=f"{prefix.capitalize()} Backtest (v1.01)",
        )
