...
v0.93 变更 (基于 v0.92):
  - 早停: 每 eval_freq 步在训练集和验证集(即 test)回测
  ...
  - 连续 patience 次不提升则停止，并恢复最佳 checkpoint

使用方式:
    python scripts/train_rl_v0.93_20260311.py --config configs/rl_config_v0.93.yaml
...
# ═══════════════════════════════════════════════════════════════════════════
# 第二部分: 交易环境 (v0.92: 支持 trade_interval 每月交易)
# ═══════════════════════════════════════════════════════════════════════════
...
class MultiStockTradingEnvV6(gym.Env):
    """v0.92: 支持 trade_interval，仅在交易步执行 action，持有步保持仓位"""
    ...
    def __init__(self, dataset: TradingDatasetV3, initial_cash=10_000_000.0,
                 ...
                 turnover_penalty=0.5, temporal_window=10, trade_interval=1):
        super().__init__()
    ...
    def step(self, action):
        steps_since_start = self.current_step - self.lookback
...
# ═══════════════════════════════════════════════════════════════════════════
# 第四部分: 训练回调 (v0.93: 含早停回调)
# ═══════════════════════════════════════════════════════════════════════════

def _evaluate_for_early_stop(env, model, norm_env=None):
    """轻量回测，返回 (dates, portfolio_values)，用于早停评估"""
    obs, _ = env.reset()
    done = False
...
def _compute_composite_score(metrics, bh_metrics=None, return_scale=5.0, alpha_scale=10.0):
    """组合指标 = (sharpe + sortino + calmar + return_term + alpha_term) / 5
    return_term = return_scale * annualized_return
    alpha_term = alpha_scale * (model_return - bh_return)
    """
    if not metrics or "error" in metrics:
        return float("-inf")
    sharpe = metrics.get("sharpe_ratio", 0.0) or 0.0
    sortino = metrics.get("sortino_ratio", 0.0) or 0.0
    calmar = metrics.get("calmar_ratio", 0.0) or 0.0
    ar = metrics.get("annualized_return", 0.0) or 0.0
    return_term = return_scale * ar
    if bh_metrics and "error" not in bh_metrics:
        bh_ar = bh_metrics.get("annualized_return", 0.0) or 0.0
        alpha = ar - bh_ar
    else:
        alpha = 0.0
    alpha_term = alpha_scale * alpha
    return (sharpe + sortino + calmar + return_term + alpha_term) / 5.0


class EarlyStoppingCallback(BaseCallback):
    """v0.93: 每 eval_freq 步回测 train/val，组合指标含 sharpe/sortino/calmar/收益率/alpha，连续 patience 次不提升则早停"""

    def __init__(self, eval_freq, patience, train_weight, val_weight,
                 train_dataset, val_dataset, env_kwargs, train_ppy, val_ppy,
                 model_dir, return_scale=5.0, alpha_scale=10.0, verbose=0):
        ...
        self.train_ppy = train_ppy
        self.val_ppy = val_ppy
    ...
    def _get_norm_env(self):
        """从当前训练环境获取 VecNormalize（评估时使用最新归一化统计）"""
    ...
    def _on_rollout_end(self):
        ...
        if self.num_timesteps < self._last_eval_at + self.eval_freq:
            return
        if self.val_dataset is None or self.val_dataset.n_dates < 3:
            return
        self._last_eval_at = self.num_timesteps

        norm_env = self._get_norm_env()
        train_env = MultiStockTradingEnvV6(dataset=self.train_dataset, **self.env_kwargs)
        val_env = MultiStockTradingEnvV6(dataset=self.val_dataset, **self.env_kwargs)

        train_dates, train_values = _evaluate_for_early_stop(
            train_env, self.model, norm_env
        )
        val_dates, val_values = _evaluate_for_early_stop(
            val_env, self.model, norm_env
        )

        train_metrics = _compute_extended_metrics(
            train_values, train_dates, self.train_ppy
        )
        val_metrics = _compute_extended_metrics(
            val_values, val_dates, self.val_ppy
        )

        train_bh_values = compute_buy_and_hold_v2(train_env)
        val_bh_values = compute_buy_and_hold_v2(val_env)
        min_train = min(len(train_values), len(train_bh_values))
        min_val = min(len(val_values), len(val_bh_values))
        train_bh_metrics = _compute_extended_metrics(
            train_bh_values[:min_train], list(train_dates[:min_train]), self.train_ppy
        )
        val_bh_metrics = _compute_extended_metrics(
            val_bh_values[:min_val], list(val_dates[:min_val]), self.val_ppy
        )

        train_composite = _compute_composite_score(
            train_metrics, train_bh_metrics, self.return_scale, self.alpha_scale
        )
        val_composite = _compute_composite_score(
            val_metrics, val_bh_metrics, self.return_scale, self.alpha_scale
        )
        combined = self.train_weight * train_composite + self.val_weight * val_composite
        ...
        if combined > self.best_score:
            ...
            if self.verbose:
                print(f"  [早停] step={self.num_timesteps} 组合={combined:.4f} (train={train_composite:.4f} val={val_composite:.4f}) *best*")
        else:
            self.no_improve_count += 1
            if self.verbose:
                print(f"  [早停] step={self.num_timesteps} 组合={combined:.4f} (train={train_composite:.4f} val={val_composite:.4f}) no_improve={self.no_improve_count}/{self.patience}")

        if self.no_improve_count >= self.patience:
            self._should_stop = True
            if self.verbose:
                print(f"  [早停] 连续 {self.patience} 次未提升，停止训练。最佳 step={self.best_timesteps} score={self.best_score:.4f}")
...
def main():
    parser = argparse.ArgumentParser(description="RL 交易模型训练 v0.93 (早停)")
    parser.add_argument("--config", type=str,
                        default=str(project_root / "configs" / "rl_config_v0.93.yaml"))
    ...
    print("=" * 70)
    print(f"  RL 交易模型 v0.93 (早停) — {run_ts}")
    print("=" * 70)
    print_gpu_info()
    ...
    early_stop_enabled = early_stop_cfg.get("enabled", False) and test_dataset is not None
    if early_stop_enabled and not test_dataset:
        print("  早停已启用但无测试集，将禁用早停")
    ...
    print(f"  时间窗口: W={W}, 交易间隔: {trade_interval}, 输入特征数: {N_MARKET_FEATURES}")
    if early_stop_enabled:
        print(f"  早停: eval_freq={early_stop_cfg.get('eval_freq', 2560)}, patience={early_stop_cfg.get('patience', 3)}")

    output_dir = project_root / "experiments" / f"rl_ppo_v0.93_{run_ts}"
    ...
    def make_env(dataset, seed=0):
        def _init():
            env = MultiStockTradingEnvV6(dataset=dataset, **env_kwargs)
            env.reset(seed=seed)
            return Monitor(env)
        return _init
    ...
    if early_stop_enabled:
        early_cb = EarlyStoppingCallback(
            ...
            train_weight=early_stop_cfg.get("train_weight", 0.5),
            val_weight=early_stop_cfg.get("val_weight", 0.5),
            ...
            return_scale=early_stop_cfg.get("return_scale", 5.0),
            alpha_scale=early_stop_cfg.get("alpha_scale", 10.0),
            verbose=1,
        )
    ...
    # v0.93: 若早停触发，恢复最佳 checkpoint
    best_path = model_dir / "ppo_trading_best.zip"
    if best_path.exists():
        print("  恢复早停最佳模型...")
        model = PPO.load(str(model_dir / "ppo_trading_best"), env=train_vec_env)
    ...
    def process_backtest(dataset, ppy, prefix):
        ...
        plot_backtest(
            ...
            title=f"{prefix.capitalize()} Backtest (v0.93)",
        )
