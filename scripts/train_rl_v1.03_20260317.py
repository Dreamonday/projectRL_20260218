...
# -*- coding: utf-8 -*-
"""
RL 交易模型训练 v1.03 — 架构升级 (跨股票注意力 + 组合上下文 + 增强Critic + 残差)
版本: v1.03
日期: 20260317

v1.03 变更 (基于 v1.02):
  - 思路1: 跨股票注意力 (Cross-Stock Attention)，股票间显式交互
  - 思路2: Actor 组合上下文，打分时注入 portfolio_embed
  - 思路3: Critic 增强，mean+max+weighted 三路聚合
  - 思路6: 残差连接，temporal encoder 与 value_mlp

使用方式:
    python scripts/train_rl_v1.03_20260317.py --config configs/rl_config_v1.03.yaml
"""
...
class TemporalScoringMlpExtractorV103(nn.Module):
    """v1.03: 跨股票注意力 + 组合上下文 + 增强Critic + 残差"""

    def __init__(self, feature_dim, n_stocks, temporal_window,
                 n_market_features=9, hidden_dim=64, n_heads=2,
                 cross_stock_heads=2, value_hidden_dim=128):
        super().__init__()
        ...
        self.cross_stock_attn = nn.MultiheadAttention(
            embed_dim, cross_stock_heads, batch_first=True, dropout=0.0
        )
        self.cross_stock_norm = nn.LayerNorm(embed_dim)

        # 思路2: Actor score_head 输入 = embed + weight + portfolio_embed (2*embed_dim + 1)
        ...
        # 思路3+6: Critic 三路聚合 + 残差
        critic_input_dim = 3 * embed_dim + N_GLOBAL_FEATURES
        self.value_fc1 = nn.Linear(critic_input_dim, value_hidden_dim)
        self.value_fc2 = nn.Linear(value_hidden_dim, value_hidden_dim)
        self.value_fc3 = nn.Linear(value_hidden_dim, embed_dim)
    ...
    def _cross_stock_encode(self, embeddings):
        """思路1: 跨股票 self-attention，序列维为 N"""
        cross_out, _ = self.cross_stock_attn(embeddings, embeddings, embeddings)
        return self.cross_stock_norm(embeddings + cross_out)

    def forward(self, features):
        temporal, current_weight, global_obs = self._parse_obs(features)
        embeddings = self._temporal_encode(temporal)
        embeddings = self._cross_stock_encode(embeddings)

        # 思路2: 组合表征
        portfolio_embed = (embeddings * current_weight).sum(dim=1, keepdim=True)
        portfolio_embed = portfolio_embed.expand(-1, self.n_stocks, -1)
        score_in = torch.cat([embeddings, current_weight, portfolio_embed], dim=-1)
        scores = self.score_head(score_in).squeeze(-1)

        # 思路3: Critic 三路聚合
        mean_pool = embeddings.mean(dim=1)
        max_pool = embeddings.max(dim=1).values
        weighted_pool = (embeddings * current_weight).sum(dim=1)
        value_input = torch.cat([mean_pool, max_pool, weighted_pool, global_obs], dim=1)

        # 思路6: value_mlp 残差
        h1 = torch.relu(self.value_fc1(value_input))
        h2 = torch.relu(self.value_fc2(h1)) + h1
        vf = self.value_fc3(h2)
        return scores, vf
    ...
    def forward_critic(self, features):
        ...
        h1 = torch.relu(self.value_fc1(value_input))
        h2 = torch.relu(self.value_fc2(h1)) + h1
        return self.value_fc3(h2)


class TemporalScoringPolicyV103(ActorCriticPolicy):
    """v1.03: 使用 TemporalScoringMlpExtractorV103"""

    def __init__(self, observation_space, action_space, lr_schedule,
                 n_stocks=100, temporal_window=10, n_market_features=9,
                 hidden_dim=64, n_heads=2, cross_stock_heads=2, value_hidden_dim=128,
                 **kwargs):
        ...
        self._cross_stock_heads = cross_stock_heads
        self._value_hidden_dim = value_hidden_dim
        super().__init__(observation_space, action_space, lr_schedule, **kwargs)

    def _build_mlp_extractor(self):
        self.mlp_extractor = TemporalScoringMlpExtractorV103(
            self.features_dim, self._n_stocks, self._temporal_window,
            self._n_market, self._hidden_dim, self._n_heads,
            cross_stock_heads=self._cross_stock_heads,
            value_hidden_dim=self._value_hidden_dim,
        )
...
class GPUCompleteEpisodeRolloutBuffer(CompleteEpisodeRolloutBuffer):
    """
    v1.02: GPU 优化的 RolloutBuffer
    - observations 存入 GPU
    - values、log_probs 保持为 tensor，不转 numpy
    - GAE 计算全程在 GPU
    - advantages、returns 保持在 GPU
    """
...
def main():
    parser = argparse.ArgumentParser(description="RL 交易模型训练 v1.03 (架构升级)")
    parser.add_argument("--config", type=str,
                        default=str(project_root / "configs" / "rl_config_v1.03.yaml"))
    ...
    print("=" * 70)
    print(f"  RL 交易模型 v1.03 (跨股票注意力+组合上下文+增强Critic) — {run_ts}")
    print("=" * 70)
    print_gpu_info()
    ...
    print(f"  v1.03: 跨股票注意力 + 组合上下文 + 增强Critic + 残差")
    ...
    output_dir = project_root / "experiments" / f"rl_ppo_v1.03_{run_ts}"
    ...
    model = PPOCompleteEpisodesV102(
        policy=TemporalScoringPolicyV103,
        env=train_vec_env,
        ...
        policy_kwargs=dict(
            ...
            cross_stock_heads=rl_cfg.get("cross_stock_heads", 2),
            value_hidden_dim=rl_cfg.get("value_hidden_dim", 128),
        ),
    ...
    def process_backtest(dataset, ppy, prefix, use_random_start=False):
        ...
        plot_backtest(
            ...
            title=f"{prefix.capitalize()} Backtest (v1.03)",
        )
