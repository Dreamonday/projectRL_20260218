...
v0.8 变更 (基于 v0.71):
  - 核心逻辑升级: Top-100 截断 + 现金锚定 (Softmax + Cash Anchor)
    1. 只对 Top-100 的股票分配权重，其余股票强制不交易。
    2. 引入分数为 0 的“现金”项参与 Softmax，实现自动仓位控制（择时）。
    3. 解决了“持仓过散”和“永远满仓”的问题。
  - 保持项: 
    - 交易成本计算保持线性 (无最低佣金/印花税复杂计算)。
    - 网络架构与数据加载与 v0.71 相同。

使用方式:
    python scripts/train_rl_v0.8_20260310.py --config configs/rl_config_v0.7.yaml
"""
...
# ═══════════════════════════════════════════════════════════════════════════
# 第二部分: 交易环境 (v0.8 核心修改: Top-100 + Cash Anchor)
# ═══════════════════════════════════════════════════════════════════════════
...
class MultiStockTradingEnvV4(gym.Env):
    ...
    核心变更: _action_to_weights
    1. 过滤无效股票
    2. 选取分数最高的 Top-100 股票，其余置为 -inf
    3. 加入 0 分现金锚点进行 Softmax
    """
    ...
    def _action_to_weights(self, action):
        ...
        if k < self.n_stocks:
            ...
            # 注意 np.argpartition 是从小到大排，所以取倒数 k 个
        ...
        # 4. 单股权重限制 (可选，但 v0.8 主要靠 Top100 和 Cash Anchor 控制)
        # 这里的归一化需要小心，因为有一个隐含的 cash_weight
        # 如果截断了股票权重，多出来的部分应该给现金，而不是分给其他股票
        
        # 简单起见，我们先按标准逻辑处理股票部分，如果有超限，削减并加回现金
        ...
        if over.any():
            ...
            # 多余的权重直接给现金 (不再分配给其他股票，防止其他股票也超限或被迫买垃圾股)
            # cash_weight_from_softmax += excess.sum() # 这里不需要显式更新，因为 step 里是 1 - sum(weights)
            
        # 这里的 weights 只是股票的权重向量，step 函数会计算 1 - weights.sum() 作为现金
    ...
    def step(self, action):
        ...
        # 现金比例自动由剩余权重决定 (含 Softmax 算出的 cash_weight + 截断溢出)
...
def run_detailed_backtest(env, model, normalize_env=None):
    ...
    while not done:
        ...
        # 遍历所有股票更新持仓 (v0.8 由于 Top100 限制，大多数 weight 会是 0)
...
def main():
    parser = argparse.ArgumentParser(description="RL 交易模型训练 v0.8")
    ...
    print("=" * 70)
    print(f"  RL 交易模型 v0.8 (Top-100 + Cash Anchor) — {run_ts}")
    print("=" * 70)
    print_gpu_info()
    ...
    # ─── 2. 输出目录 ───
    output_dir = project_root / "experiments" / f"rl_ppo_v0.8_{run_ts}"
    ...
    def process_backtest(dataset, ppy, prefix):
        ...
        plot_backtest(
            ...
            title=f"{prefix.capitalize()} Backtest (v0.8 Top-100 + Cash Anchor)",
        )
        return metrics
